...
class DiracHelper(context: Context) {
    ...
    private fun updateVolume(volume: Int) {
        ...
        val normalizedVolume = when {
            ...
            volume <= 0 -> -1.0f
            volume > DEFAULT_VOLUME -> (volume - DEFAULT_VOLUME).toFloat() / (maxVolume - DEFAULT_VOLUME).toFloat()
            else -> (volume - DEFAULT_VOLUME).toFloat() / DEFAULT_VOLUME.toFloat()
        }.coerceIn(-1.0f, 1.0f)

        Log.d(TAG, "updateVolume: norm=$normalizedVolume (vol=$volume/$maxVolume)")
    ...
    companion object {
        ...
        private const val DEFAULT_VOLUME = 18
